...
> 与 overseer 差异:overseer 是**进程内库**且**不自动重启崩溃**(同码退出);lode 是**外部通用监督器**,默认**保活**(重试后暂停、绝不让容器崩溃重启),并提供 `off`(镜像)+ 回滚,是其超集。**零停机为可选高级特性,v1 默认 `stop-start`,socket-activation/overlap 后续按需启用。**
