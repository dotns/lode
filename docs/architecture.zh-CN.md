...
> 通用镜像 = `distroless/static` + lode 二进制(无需任何语言运行时);一个实例 = **单程序、单通道**。
...
- **为什么 Rust**:Bun 编译版 lode ≈ 91MB(内嵌 JS 运行时),镜像还得拖着它;Rust 静态(musl)≈ 几 MB,镜像可 `FROM scratch`/distroless,这才是真正的"语言无关 + 小"。
- **关系**:`通用镜像(distroless + lode)` + `远程 manifest/签名包` → 容器启动即按策略校验并运行应用。换应用 = 换 `[update].manifest`(env `LODE_MANIFEST`),镜像零重建。
...
```
通用镜像(构建一次)         ┌──────────────────────────────────┐
distroless/static          │  lode (Rust 静态二进制, 几 MB)   │
                          └────────────────┬─────────────────┘
...
- **镜像**:`FROM gcr.io/distroless/static`(带 CA 证书,HTTPS 拉包省心)→ `COPY lode /lode` → `ENTRYPOINT ["/lode"]`。app 自带运行时或本身就是静态二进制 → 镜像可极小。
...
当 `run`/`exec` 依赖某运行时(如 `bun`)时,可在 `lode.toml` 的 `[runtime]` 声明:lode 先查 PATH,**缺失则从 `download` 下载**(与 artifact 同样支持 `format` 解包、`sha256`/`sig` 校验),放到 `$DATA_DIR/runtime/` 并把该目录**前置到子进程 PATH**。自包含二进制则省略此表。

```toml
[runtime]
runtime  = "bun"                                    # run/exec 用到的可执行名
download = "https://example.com/bun-linux-x64.zip"  # PATH 上找不到 bun 时下载
...
- **`lode`(裸跑)= 启动并监督服务**:加锁(单实例)→ 确定/安装版本(+ 必要时下载运行时)→ 跑 **`run`**(lode.toml,自动补 `{entry}`)→ 监督(按 `supervise.restart` 策略,默认镜像子进程)→ 按策略轮询热更/回滚 → 信号透传如上。适合 server/daemon。镜像 `ENTRYPOINT ["/lode"]`,`docker run img` 即启动。
...
| `LODE_RUNTIME_DOWNLOAD` | `--runtime-download <url>` | `runtime.download` | — | 运行时缺失时的下载地址 |
...
子进程环境:透传宿主环境并**剥离配置类 `LODE_*`**;注入只读自省变量 `LODE_ACTIVE_VERSION`、`LODE_DATA_DIR`、`LODE_INSTANCE`(本次启动唯一号,用于就绪握手,§8)。
...
- `versions`(必填):对象,键为版本 id(被通道 `latest` 引用),值含 `min_lode`/`notes`(可选)+ `assets` 数组(≥1)。
...
lode 是 **multi-call 二进制**,按 `argv[0]` 分流:`lode-cli` 是同一二进制的**软链接**,随二进制一同发布;镜像内 `/lode` 与 `/lode-cli` 均可用。
...
- `Dockerfile` —— 通用镜像(`FROM gcr.io/distroless/static` + `COPY lode`);多阶段构建(musl 编译 → 拷贝)。
